...
* about 832 Gameplay Cards with at least one text revision
  ...
  * 841 `test_validated_executable` definitions
  * 136 timing-only `manual_resolution` definitions
  * registry-entry executable coverage is currently 86.08%
...
The human-readable effect verification artifact now contains 46 Japanese / Chinese scenarios. Seven scenarios execute focused state transitions, while 39 contract scenarios verify exact trigger, choice, operation, support status, official Japanese text, and card visual linkage. These checks are implementation evidence only; `reviewed_executable` still requires human rule review.

The latest skill-dense regression uses 30 generated decks across 100 matches. It completed 98 matches; the other two reached the 600-action cap at turns 36 and 37. Across the run, 689 executable effects were triggered and all 689 completed. There were no illegal actions, silent effect skips, or replay errors. The remaining failures are therefore classified as long-game sandbox-policy limits rather than effect-execution blockers.
