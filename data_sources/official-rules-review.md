...
| `stage_attachments` | Rules 4.5.5 and 10.5.3-10.5.5 allow Member and Energy cards under a top Member. Attached cards have no orientation. When the top Member leaves the Stage, attached Members move to Waiting Room and attached Energy returns to the Energy Deck. | GameState keeps the top Member separate from a per-area attachment collection. Attached Energy is not part of the Energy Area and cannot pay costs. |
| `stage_position_changes` | Rules 11.10 and 11.11 define Position Change and Formation Change. Position Change swaps with an occupied destination; Formation Change atomically assigns all current Stage Members without placing two in one area. | Stage movement must move each top Member together with its attached cards and must be represented by replay-safe structured Actions. |
