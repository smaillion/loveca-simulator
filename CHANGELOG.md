...
- Phase 5 effect registry を 977 件中 802 件の `test_validated_executable` まで拡張し、registry entry ベースの coverage を 82.09% に更新。
...
- effect registry 全体は 977 件中 802 件の `test_validated_executable`、175 件の `manual_resolution` となり、registry entry ベースの coverage は 82.09% に更新。
...
- `PL!HS-bp6-014:1` は、対象の「藤島 慈」または「大沢瑠璃乃」が Stage にいない場合でも、手札から控室へ置くコストと 1 枚抽牌を処理し、Blade 付与だけを空解決できるようにした。
- `PL!HS-bp6-006` は、手札中の cost reduction、みらくらぱーく！以外との Baton replacement 制限、Live 成功時の Wait + 次 Active Phase 非アクティブ化を構造化した。
- 二段階効果が cost / branch / auto action 後の follow-up choice に入る時、`effect_choice_started` event を生成し、UI / replay / sandbox report で「続きの選択待ち」を確認できるようにした。
- effect-focused な human-readable verification report を追加し、`PL!HS-bp6-014:1`、`PL!HS-bp6-006`、`PL!HS-bp2-026`、Baton Touch 二重防止、`PL!HS-sd1-005` の重点修正シナリオを日文 / 中文 Markdown と公式画像付きで確認できるようにした。
...
- `source_moved_this_turn` が top-level condition にある自動効果を正しく gating し、移動していない source Member による Live 成功時 bonus が誤発動しないようにした。
- static Heart / Blade / numeric bonus の計算時に operation 側 condition を評価し、6 Energy 条件だけで 8 Energy bonus まで加算される問題を防止。
- Yell 公開 Live 枚数参照の `amount_source=revealed_live_count` は、既存の Live 判定公開カードを参照し、必要に応じて上限 cap を適用できるようにした。
- 82.09% coverage 更新後の `10 decks x 5 matches --manual-policy block --max-actions 220` smoke は 4 完走 / 1 `max_actions:member_development_at_cap` で、`mandatory_manual_resolution`、skipped effects、`illegal_action` は 0。
...
- 将 Phase 5 effect registry 扩展到 977 条中的 802 条 `test_validated_executable`，按 registry entry 计算覆盖率更新为 82.09%。
...
- effect registry 全体更新到 977 条，其中 802 条 `test_validated_executable`、175 条 `manual_resolution`，按 registry entry 计算的覆盖率为 82.09%。
...
- `PL!HS-bp6-014:1` 在场上没有「藤島 慈」或「大沢瑠璃乃」时也能从手牌发动，正常把自身放入控室并抽 1 张，Blade 赋予目标为空时只跳过该 modifier。
- `PL!HS-bp6-006` 已结构化支持手牌中 cost reduction、非みらくらぱーく！Baton replacement 限制、Live 成功时 Wait + 下一 Active Phase 不转 Active。
- 两段式技能在 cost / branch / auto action 后进入后续选择时，会生成 `effect_choice_started` event，方便 UI、Replay 和 sandbox report 确认技能正在等待后续选择。
- 新增 effect-focused 的人类可读 verification report，可用日文 / 中文 Markdown 加官方图片检查 `PL!HS-bp6-014:1`、`PL!HS-bp6-006`、`PL!HS-bp2-026`、Baton Touch 二次登场阻止、`PL!HS-sd1-005` 等重点修正场景。
...
- 修复 `source_moved_this_turn` 位于 top-level condition 的自动技能没有正确 gating，导致未移动 source Member 也触发 Live 成功 bonus 的问题。
- static Heart / Blade / numeric bonus 计算现在会检查 operation 侧 condition，避免只满足 6 Energy 条件时错误获得 8 Energy bonus。
- `amount_source=revealed_live_count` 现在可直接参照 Live 判定中已经公开的应援卡，并支持上限 cap。
- 82.09% 覆盖率更新后的 `10 decks x 5 matches --manual-policy block --max-actions 220` smoke 为 4 局完走 / 1 个 `max_actions:member_development_at_cap`，`mandatory_manual_resolution`、skipped effects 和 `illegal_action` 均为 0。
